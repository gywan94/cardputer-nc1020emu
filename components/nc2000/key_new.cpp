...
void SetKeyWayback(int code_y,int code_x, bool down_or_up){
  ...
  // cardputer: warm_reset_if_clkoff is already handled in main.cpp wqx_set_key() via G0 button.
  // Do NOT trigger warm reset on every function key press — that resets the system
  // every time the user presses any Fn key, preventing normal operation and save_state.
