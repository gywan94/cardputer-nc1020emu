...
{
#ifdef NC2K_SD_PAGED
    /* NOR is paged: there is no full nor_buff to serialize; just flush dirty
     * slots back to the live .nor file. (Save-as to a different path isn't
...
void init_nor(){
#ifdef NC2K_SD_PAGED
    /* No PSRAM on Cardputer: NOR lives in the .nor file and is paged in 32 KB
     * banks. sdpg_init() allocates the shared slot pool + opens/creates .nor.
     * nor_buff stays null; nor_banks[] is unused (GetBank/bbs call sdpg_nor). */
    if(!sdpg_init(nc2k_rom.norFlashPath.c_str(), num_nor_pages)){
        printf("[nor] sd paging init failed!\n");
    }
    return;
#else
#ifdef TAB5_PORT
...
#ifdef NC2K_SD_PAGED
    extern bool g_memmap_is_nor[8];
    return g_memmap_is_nor[addr >> 13];
#else
...
#ifdef NC2K_SD_PAGED
            sdpg_mark_dirty_ptr(memmap[addr>>13]);
#endif
            fp_step = 4;
