...
void init_rom(){
#ifdef NC2K_SD_PAGED
    /* No PSRAM: the 12 MB ROM stays on SD and is paged in 32 KB banks. Just open
     * + size-check the file; GetBank() resolves banks via sdpg_rom(). The
     * rom_buff / rom_volume* tables are left unused. */
    if(!sdpg_open_rom(nc2k_rom.romPath.c_str())){
        printf("[rom] sd paging open failed!\n");
    }
    return;
#endif
#ifdef TAB5_PORT
