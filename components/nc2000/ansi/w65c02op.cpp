...
#include "cpu.h"
#include "comm.h"
#include "esp_attr.h"   /* IRAM_ATTR — try placing the hot interpreter in IRAM */
...
IRAM_ATTR DWORD CpuExecuteOP(DWORD total, int max_cycles)
{
     ...
     * so the compiler reloaded them after every memory write. Locals can't be
     * aliased -> they stay in host registers. Flags (mN..mC) stay global because
     * PS()/setPS() touch them; only IRQ needs a sync (it pushes PC via SP). */
    int mPC = ::mPC, mA = ::mA, mX = ::mX, mY = ::mY, mSP = ::mSP;
    int mOperand = ::mOperand, mOpcode = ::mOpcode;
    do {
    if (g_irq && !mI) {
        g_irq = false;
        ::mPC=mPC; ::mA=mA; ::mX=mX; ::mY=mY; ::mSP=mSP; ::mOperand=mOperand; ::mOpcode=mOpcode;
        total += CpuExecuteIRQ();
        mPC=::mPC; mA=::mA; mX=::mX; mY=::mY; mSP=::mSP; mOperand=::mOperand; mOpcode=::mOpcode;
    }
    DWORD cycle = 0;
...
//     }
    ...
    //
    if (g_wai) { total = (DWORD)max_cycles; if ((int)total <= 0) total = 1; break; }
...
DWORD CpuExecute(void){
    DWORD cycle = CpuExecuteOP(0, 0);   /* legacy CPU_RUN1 path: one instruction */
    if (g_nmi) {
