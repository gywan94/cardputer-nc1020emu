...
#ifdef NC2K_SD_PAGED
	/* Batched: the per-instruction fetch+switch loop lives inside CpuExecuteOP so
	 * the 6502 registers (mPC/mA/... globals) stay in host registers across the
	 * whole batch instead of being spilled around a cross-TU call every insn. */
	cycle = CpuExecuteOP(cycle, max_cycles);
#else
    do{
		if (g_irq && !mI){
			g_irq = false;
