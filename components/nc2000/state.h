...
struct nc2k_states_t{
	...
	/* nc1020-only Cardputer build: ext_ram is only touched by the abnormal
	 * bs=0,roa=1 case (mem.cpp), which uses just the low 16 KB. Halving it frees
	 * 16 KB for more SD-paging slots. (nc2000's full-32KB use is dead here.)
	 * NOTE: kept unconditional — main/ and nc2000/ must agree on struct layout. */
	uint8_t ext_ram[0x4000];
