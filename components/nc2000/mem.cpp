...
#ifdef NC2K_SD_PAGED
/* Per-8KB-page "is this a NOR slot" flag, recomputed once per bus switch so the
 * hot path (in_nor_range, called on every Load/Store) is an O(1) lookup instead
 * of scanning all slots. memmap only changes inside super_switch(). */
bool g_memmap_is_nor[8] = {false};
#endif
...
void init_mem(){
	...
	memmap[3] = ram06;
	memmap[4] = ram08;
	memmap[5] = ram08 + 0x2000;
	memmap[6] = ram08 + 0x4000;
	memmap[7] = ram08 + 0x6000;
	// 暂时不调用 super_switch()，避免访问未初始化的指针
}
...
#ifdef NC2K_SD_PAGED
	/* Resolve the selected bank's four 8 KB sub-pages directly (no contiguous
	 * 32 KB). Mirrors GetBank()'s nor/rom selection; nc1020-only in paged build.
	 * sub0->memmap[4](+0) sub1->[5](+0x2000) sub2->[2](+0x4000) sub3->[3](+0x6000) */
	{
		uint8_t volsel = ram_io[0x0D] & 0x03;
		int vol = (volsel==1) ? 1 : (volsel==2 ? 2 : 0);
		if (bank_idx < num_nor_pages) {
			memmap[4] = sdpg_nor_sub(bank_idx,0);
			memmap[5] = sdpg_nor_sub(bank_idx,1);
			memmap[2] = sdpg_nor_sub(bank_idx,2);
			memmap[3] = sdpg_nor_sub(bank_idx,3);
		} else if (bank_idx >= 0x80 && nc1020mode) {
			memmap[4] = sdpg_rom_sub(vol,bank_idx,0);
			memmap[5] = sdpg_rom_sub(vol,bank_idx,1);
			memmap[2] = sdpg_rom_sub(vol,bank_idx,2);
			memmap[3] = sdpg_rom_sub(vol,bank_idx,3);
		} else {
			return;   /* invalid bank (matches GetBank()==NULL) */
		}
	}
#else
...
#ifdef NC2K_SD_PAGED
	/* nc1020/nc2000/nc3000: NOR is SD-paged. Only nor[0] (memmap[7]) and
	 * nor[bbs_idx>>2] (memmap[6]) are ever read by the CPU, so resolve just
	 * those two instead of nor_banks[0..3]. (pc1000 uses ROM volumes — falls
	 * through to the legacy path below.) */
	if(nc2000mode||nc3000mode||nc1020mode){
		/* suboff[sub]>>13 = sub-page index: {0x4000,0x6000,0x0000,0x2000}->{2,3,0,1} */
		static const uint8_t subpg[4] = {2,3,0,1};
		uint8_t bbs_idx = ram_io[0x0A]&0x0f;
		memmap[7] = sdpg_nor_sub(0, 3);                       /* nor[0] + 0x6000 */
		memmap[6] = sdpg_nor_sub(bbs_idx>>2, subpg[bbs_idx & 3]);
		if(bbs_idx == 1){
			if(nc1020mode||nc2000mode) memmap[6] = ram04;
			if(nc3000mode)             memmap[6] = ram06;
		}
		return;
	}
#endif
...
void super_switch(){
...
#ifdef NC2K_SD_PAGED
	sdpg_switch_begin();   /* pin every bank resolved during this switch */
#endif
	SwitchCheck();
	Switch0x2000();
	SwitchBbsBios_67();
	...
	SwitchZP40();
#ifdef NC2K_SD_PAGED
	/* cache which mapped pages are NOR (only [2..7] can be) for O(1) in_nor_range */
	g_memmap_is_nor[0] = g_memmap_is_nor[1] = false;
	for(int i=2;i<8;i++) g_memmap_is_nor[i] = sdpg_ptr_is_nor(memmap[i]);
#endif
}
